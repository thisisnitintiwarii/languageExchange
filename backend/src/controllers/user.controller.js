...
export async function getMyFriends(req, res) {
    try {
        ...
        const user = await User.findById(currentUserId).select("friends")
            .populate(
                "friends", "fullName profilePick nativeLanguage learningLanguage"
            )
...
export async function getFriendRequests(req, res) {
    try {
        ...
        const incomingReqs = await friendRequest.find({
            ...
            status: "pending"
        }).populate("sender", "fullName profilePick nativeLanguage learningLanguage")

        const acceptedReqs = await friendRequest.find({
            recipient: req.user.id,
            status: "accepted"
        }).populate("sender", "fullName profilePick ")
...
export async function getOutgoingFriendReqs(req, res) {
    try {
        ...
        const outgoingReqs = friendRequest.find({
            sender: req.user.id,
            status: "pending"
        }).populate("recipient", "fullName profilePick nativeLanguage learningLanguage")
